...
public class SubtitleService : ISubtitleService
{
    ...
    /// </summary>
    public async Task<SubtitleTrackDto> UploadSubtitleInternalAsync(
        Guid lessonId,
        string language,
        string label,
        IFormFile file,
        ...
        string kind = "subtitles")
    {
        try
        {
            ...
            // NOTE: No permission check - this is for internal/system use only
            ...
            _logger.LogInformation("[INTERNAL] Uploading subtitle file: {FileName} ({Size} bytes) for lesson {LessonId} in language {Language}",
                file.FileName, file.Length, lessonId, language);
            ...
            var subtitleTrack = new SubtitleTrack
            {
                ...
                CueCount = cueCount,
                DurationSeconds = durationSeconds,
                CreatedByUserId = null,  // System generated, no user
                IsAutoGenerated = true,
                GenerationSource = "auto-generate-job"
            };

            // Save to SQL Server
            var savedTrack = await _subtitleRepository.AddAsync(subtitleTrack);

            _logger.LogInformation("[INTERNAL] Subtitle track created: {Id} for lesson {LessonId} in language {Language} (GridFS ID: {FileId})",
                savedTrack.Id, lessonId, language, fileId);
        ...
        {
            _logger.LogError(ex, "[INTERNAL] Error uploading subtitle for lesson {LessonId}", lessonId);
            throw;
        }
    }
