...
echo "🎬 Creating REAL MP4 Test Videos for InsightLearn"
echo "=================================================="
echo ""
echo "Configuration:"
echo "  - Videos: $TOTAL_VIDEOS × ${DURATION_SECS}s (720p)"
echo "  - Output: $VIDEO_DIR"
echo ""

# Check FFmpeg
if ! command -v ffmpeg &> /dev/null; then
    echo "❌ FFmpeg not installed. Install with: sudo dnf install ffmpeg"
    exit 1
fi
...
# ============================================
echo "📦 Generating $TOTAL_VIDEOS real MP4 video files..."
mkdir -p "$VIDEO_DIR"
...
for i in $(seq 1 $TOTAL_VIDEOS); do
    ...
    if [ ! -f "$FILE" ]; then
        ...
        if file "$FILE" | grep -q "MP4"; then
            ...
            echo "    ❌ Failed to create valid MP4: $FILE"
        fi
    else
...
echo ""
echo "✅ All video files generated"
echo ""
...
# ============================================
echo "📚 Creating course with real videos..."
...
if [ -z "$COURSE_ID" ] || [ "$COURSE_ID" = "null" ]; then
    echo "❌ Failed to create course: $COURSE_RESPONSE"
    exit 1
fi
...
for i in $(seq 1 $TOTAL_VIDEOS); do
    ...
    if [ -f "$VIDEO_FILE" ]; then
        ...
        if [ "$HTTP_CODE" = "200" ] || [ "$HTTP_CODE" = "201" ]; then
            ...
            UPLOADED=$((UPLOADED + 1))
            echo "  ✅ Video $i uploaded (ID: $VIDEO_ID)"
            ...
            echo "  ❌ Video $i failed (HTTP $HTTP_CODE)"
        fi
    fi
done
...
echo ""
echo "🧹 Keeping temp files at: $VIDEO_DIR"
